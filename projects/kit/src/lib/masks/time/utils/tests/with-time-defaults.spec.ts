...
describe('withTimeDefaults', () => {
    describe('separators resolution', () => {
        it('fills missing positions with canonical separators from mode', () => {
            expect(
                maskitoWithTimeDefaults({mode: 'HH:MM:SS.MSS', separators: []})
                    .separators,
            ).toEqual([':', ':', '.']);
        });
        ...
        it('defaults to canonical separators when no separators provided', () => {
            expect(maskitoWithTimeDefaults({mode: 'HH:MM:SS.MSS'}).separators).toEqual([
                ':',
                ':',
                '.',
            ]);
        ...
        it('ignores meridiem part when computing canonical separators', () => {
            expect(maskitoWithTimeDefaults({mode: 'HH:MM AA'}).separators).toEqual([':']);
        });

        it('returns empty array for mode without any separator slot', () => {
            expect(maskitoWithTimeDefaults({mode: 'HH'}).separators).toEqual([]);
            expect(maskitoWithTimeDefaults({mode: 'HH AA'}).separators).toEqual([]);
        });
    });

    describe('dayPeriod resolution', () => {
        it("defaults to ['', ''] for 24-hour mode without explicit dayPeriod", () => {
            expect(maskitoWithTimeDefaults({mode: 'HH:MM'}).dayPeriod).toEqual(['', '']);
        });

        it("defaults to ['AM', 'PM'] for deprecated 'AA' mode (backward compat)", () => {
            expect(maskitoWithTimeDefaults({mode: 'HH:MM AA'}).dayPeriod).toEqual([
                'AM',
                'PM',
            ]);
            expect(
                maskitoWithTimeDefaults({mode: 'HH:MM AA', dayPeriod: ['', '']})
                    .dayPeriod,
            ).toEqual(['AM', 'PM']);
        });

        it('treats explicit dayPeriod as 12-hour for time-segment bounds', () => {
            const result = maskitoWithTimeDefaults({
                mode: 'HH:MM',
                dayPeriod: ['AM', 'PM'],
            });
